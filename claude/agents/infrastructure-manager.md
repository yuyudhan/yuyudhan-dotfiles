...
description: Use this agent when working with Docker, Docker Compose, Nginx configurations, deployment setups, environment variables, or Make commands. Trigger this agent for: service startup failures, container health issues, port binding problems, environment variable mismatches, hot reload setup for dev environments, .env file synchronization, Nginx routing/proxy issues, inter-service communication problems, or Make command debugging. Examples:\n\n<example>\nContext: User is setting up a new Docker Compose configuration for a web application with hot reload.\nuser: "I need to set up Docker Compose for my Node.js backend with hot reload enabled"\nassistant: "I'll use the infrastructure-manager agent to create a properly configured Docker Compose setup with hot reload."\n<uses infrastructure-manager agent>\n</example>\n\n<example>\nContext: User encounters container startup failure.\nuser: "My database container keeps crashing on startup"\nassistant: "Let me use the infrastructure-manager agent to diagnose the container issues and check the logs."\n<uses infrastructure-manager agent>\n</example>\n\n<example>\nContext: User has .env and .env.example files out of sync.\nuser: "I added some new environment variables but forgot to update .env.example"\nassistant: "I'll use the infrastructure-manager agent to synchronize your .env and .env.example files and ensure Docker Compose is pulling values correctly."\n<uses infrastructure-manager agent>\n</example>\n\n<example>\nContext: User needs to optimize their Docker Compose configuration.\nuser: "My Docker Compose file has hardcoded ports and database credentials"\nassistant: "I'll launch the infrastructure-manager agent to refactor your Docker Compose configuration to use .env variables properly."\n<uses infrastructure-manager agent>\n</example>
model: sonnet
color: yellow
---
